...
class Window(QtWidgets.QDialog):
    ...
    def UpdatePL_TVOM(self):
        ...
        Input:
            k = current step
            TVOM = time value of money is considered
        Output:
            An updated PL matrix up to the current step as a side effect
        '''
        k = self.simCurrentStep
        r = self.r # current interest rate
        deltaT = self.T_C / self.simSteps
        if self.radio_ignore_TVOM.isChecked():
            # copy self.PL to self.PL_TVOM
            self.PL_TVOM = list(self.PL)
        else: # ignore TVOM
            for row in range(k+1):
                for col in range(3):
    ...
    def showGraph(self):
        ...
        #x      = np.linspace(60, 140, 51)
        x      = np.linspace(s1_x_min, s1_x_max, 51)
        y_call = bsm.bs_call_price(x, K_C, r, q, σ, T_C*(1-(self.simCurrentStep)/(self.simSteps)))
        y_put  = bsm.bs_put_price(x, K_P, r, q, σ, T_P*(1-(self.simCurrentStep)/(self.simSteps)))
        y      = α * y_call + β * y_put
        ...
        # plot circle on s2
        put_circle_x = self.circleCoords[self.simCurrentStep][0]
        put_circle_y = bsm.bs_put_price(put_circle_x, K_P, r, q, σ, T_P*(1-(self.simCurrentStep)/(self.simSteps)))
        ...
        call_circle_x = self.circleCoords[self.simCurrentStep][0]
        call_circle_y = bsm.bs_call_price(call_circle_x, K_C, r, q, σ, T_C*(1-(self.simCurrentStep)/(self.simSteps)))
        ...
        upperLimit = int((len(self.X)-1) * self.simCurrentStep / self.simSteps)
        ...
        s4.plot(self.Xdiscrete[:self.simCurrentStep+1], self.Ydiscrete[:self.simCurrentStep+1], 'black', lw=1.25, label='price')
        ...
        for i in range(0, self.simCurrentStep+1):
        ...
        for i, point in enumerate(self.circleCoords):
            ...
            elif i > 0 and i <= self.simCurrentStep: # skip step 0, i.e. when there's only one curve
        ...
        #s2.plot(returns, 'r', lw=2.5, label='present value')
        ...
        profit_loss = self.tablePL
        profit_loss.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        profit_loss.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        profit_loss.setColumnCount(4)
        profit_loss.setRowCount(6)
        #hori_labels = ['1P Value', '1C Value', 'xC+yP Value', 'P/L']
        hori_labels = ['1P\nValue', '1C\nValue', 'xC+yP\nValue', 'P/L']
        profit_loss.setHorizontalHeaderLabels(hori_labels)
        font = QFont('Segoe UI', 9)
        font.setBold(True)
        profit_loss.horizontalHeader().setFont(font)
        vert_labels = [f"t{str(j)}" for j in range(6)]
        profit_loss.setVerticalHeaderLabels(vert_labels)
        font = QFont('Segoe UI', 9)
        font.setBold(False)
        profit_loss.verticalHeader().setFont(font)
        # Headers
        profit_loss.resizeColumnsToContents()
        header = profit_loss.horizontalHeader()
        header.setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)
        for col in range(4):    
            header.setSectionResizeMode(col, QtWidgets.QHeaderView.Stretch)

        # update P/L table: fill cell with values
        #for row in range(self.simCurrentStep+1):
        k = self.simCurrentStep 
        profit_loss.clearSelection()
        # 1P value
        ...
        # Select a row
        profit_loss.setRangeSelected(QTableWidgetSelectionRange(k, 0, k, 3), True)
        for row in range(6):
            profit_loss.setRowHeight(row, 20)
        ...
        xPL = self.Xdiscrete
        option_val_t0 = self.PL[0][2]  # option value at t0
        yPL = [self.PL[row][2] for row in range(k+1)]
        ...
        for row in range(0, k+1):
            s5.annotate(f"{self.PL[row][3]:+.2f}", xy=[self.Xdiscrete[row]-0.1, self.PL[row][3]+option_val_t0+2])
        for row in range(k+1):
            s5.plot(self.Xdiscrete[row], self.PL[row][3]+option_val_t0, marker='o', color='black')
        #s5.annotate(f"{self.PL[0][3]:.2f}", xy=[0.5, 25])

        #profit_loss.setStyleSheet("QTableView::item:selected { color:red; background:yellow; font-weight: bold; } " + "QTableView::vertical:header {font-size: 9}")
